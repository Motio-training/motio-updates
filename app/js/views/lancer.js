...
import { drawMuscleMap, drawLegend, MuscleScale, musLabel } from '../muscle-map.js';
...
export async function vueLancerSeance(params) {
  ...
  /**
   * Bilan de fin de séance — SummaryScreen (TrainingScreens.kt ~2745) :
   * charge totale, temps global/sous tension, carte musculaire, détail
   * modifiable par exercice, proposition de mise à jour du modèle si la
   * séance jouée s'écarte de la définition, partage.
   */
  async function afficherBilan() {
    ...
    const el = h(`
      <section class="page page-etroite">
        ...
        <h1>${esc(session.workoutName || `Séance ${session.category}`)}</h1>
        ...
        <div class="bilan-petites">
          <div class="bilan-petite"><span class="label">Temps global</span><span class="valeur">${esc(duree((session.endedAt - session.startedAt) / 1000))}</span></div>
    ...
    el.querySelector('[data-fermer]').onclick = () => { location.hash = '#/seances'; };
